@echo off
set version=3.1
title Mwk Ambrose Tweaks - %version%
...
:: Batch File Log
echo Ancel's Performance Batch Log > Mwk Ambrose_Log.txt

:: Enable ANSI Escape Sequences
reg add "HKCU\CONSOLE" /v "VirtualTerminalLevel" /t REG_DWORD /d "1" /f >> Mwk Ambrose.txt
echo.
...
echo                   ████████████████████████████████                       3 Better VRAM Creation
echo                  ▄██████████▀▀        ▀▀██████████▄                      4 Cache + Log Cleaning
echo                 ▄█████████▀        █    ▀██████████▄                     5 Power Plan Tweak
echo            ▄▄▄▄██████████▀        █▀      ▀██████████▄▄▄▄                6 Regedit Tweaks
echo           ███████████████       ▄██        ███████████████               7 Internet Tweaks
echo           ███████████████      ██████      ███████████████               8 Timer Resolution Services
echo           ███████████████         ██▀      ███████████████               9 Game Specific Tweaks
echo            ▀▀▀▀██████████▄       █▀       ▄██████████▀▀▀▀                10 Server Changer
echo                 ▀█████████▄     █▀       ▄█████████▀                     11 System Bugfixes
echo                   ██████████▄▄        ▄▄██████████                       12 CPU Tweaks
echo                  ▄████████████████████████████████▄                      13 GPU Tweaks
echo                 ▄██████████████████████████████████▄                     14 Mouse Tweaks
echo                ██████████████████████████████████████                    15 Services Optimization
echo                ▀███████▀▀▀████████████████▀▀▀███████▀                    16 Incredibily Small File
echo                  ▀███▀       ▀████████▀       ▀███▀                      17 Automatic Installation
echo                               ████████                                   18 Debloater
echo                               ████████                                   19 And many more...
echo                               ████████                                   
echo                                ▀▀▀▀▀▀                                                                   
echo                                     %c%Thank you for using Mwk Ambrose Tweaks%t%    
echo                                                    %q%Version: %Version%%q%%c%
pause >nul                                               
...
echo.
echo %g%For free %g%
echo  %g%github:%g%.s https://github.com/mwkambrose  %g%
echo.
echo.
echo.
echo.
...
echo.
echo  %r%Red%u%, %gold%Gold%u%, %m%Pink%u%, %p%Purple%u%, %lime%Lime%u%,
echo.
echo.
set /p preset= 
...
echo.
echo.
echo                                 %c% Mwk Ambrose is a tweaking script that optimizes your system%u% 
...
echo.
echo.
echo      %c%Created by  Mwk Ambrose%u%
echo %c%This is a GUI for the  Mwk Ambrose Tweaks%u%
echo.
...
echo.
echo.
echo %c% Mwk Ambrose Tweaks is a tweaking script that optimizes your%u% 
...
echo %r%Mwk\Mwk Ambrose Folder\Important - Read First.txt%u% 
echo.
echo %g%======PRESS ANY KEY TO CONTINUE======
...
echo.
echo.
start https://github.com//MwkAmbrose 
echo.
echo %g%======PRESS ANY KEY TO CONTINUE======
...
powershell Invoke-WebRequest "https://cdn.discordapp.com/attachments/1145945307205611623/1219858329312952400/BatlezTweaks.pow?ex=66a86599&is=66a71419&hm=c95fa894df1850327db4d4d910d9ec4691da35ca3ccc3bc10c96d6476f0438cd&" -OutFile "%temp%\Mwk Ambrose_Tweaks.pow"
cls
...
del "%temp%\Mwk Ambrose_Tweaks.pow"
echo.
echo %g%======PRESS ANY KEY TO CONTINUE======
...
wmic process where name="svchost.exe" CALL setpriority "normal"
...
Reg.exe add "HKLM\SOFTWARE\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_MAXCONNECTIONSPER1_0SERVER" /v "iexplorer.exe" /t REG_DWORD /d "10" /f
...
netsh interface ipv6 set subinterface "Ethernet 2" mtu=1500 store=persistent
netsh interface ipv4 set subinterface "Ethernet 3" mtu=1500 store=persistent
netsh interface ipv6 set subinterface "Ethernet 3" mtu=1500 store=persistent
...
Reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Policies\Microsoft\Windows\Psched" /v "NonBestEffortLimit" /t REG_DWORD /d "0" /f
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v GlobalMaxTcpWindowSize /t REG_DWORD /d 256960 /f 
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters" /v DisableDynamicDiscovery /t REG_DWORD /d 0 /f 
...
timeout /t 1 /nobreak > NULfor /f "tokens=3*" %%s in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\NetworkCards" /f "ServiceName" /s^|findstr /i /l "ServiceName"') do (
	reg add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces\%%s" /v "TcpAckFrequency" /t REG_DWORD /d "1" /f
 	reg add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces\%%s" /v "TCPNoDelay" /t REG_DWORD /d "1" /f
 	reg add "HKLM\SYSTEM\CurrentControlSet\Services\Tcpip\Parameters\Interfaces\%%s" /v "TcpDelAckTicks" /t REG_DWORD /d "0" /f
	reg.add "HKLM\SYSTEM\CurrentControlSet\Services\Psched\Parameters\Adapters\%%s" /v "NonBestEffortLimit" /t REG_DWORD /d "0" /f
...
echo.
echo %c%                       ╔══════════════════════════════════════════════════╗ %u%
echo                        %c%║%u%           [%c%1%u%] Mwk Ambrose Toolbox                     %c%║%u% 
...
echo.
echo                                    - Thank you for using Mwk Ambrose Toolbox! -
echo _______________________________________________________________________________________________________________________
echo.
echo                    ===========================================================================
echo                    " For Mwk Ambrose Toolbox - please note that this is currently in development! "
echo                    ===========================================================================
echo                    ===========================================================================
echo                    "  Currently working on every Windows build. Please be aware of any bugs  "
echo                    ===========================================================================
echo.
echo                                     %r%Press any key to start Mwk Ambrose Toolbox! %u%
pause >nul
...
:89
cls
title Mwk Ambrose Tweaks - Mwk Ambrose  Toolbox
...
:Destruct
title Thanks for using Mwk Ambrose Tweaks!
cls
echo.            
echo %u%Developed by: %c% tools box 
echo %u%Github: %c%https://github.com/MwkAmbrose
timeout /t 5 >nul
exit
